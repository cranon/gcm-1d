...
def foo1(NumT, maxTau):
	os.system('./run -n ' + str(NumT) + ' -t ' + str(maxTau))
...
thrd1 = threading.Thread(target = foo1, args = (NumT, maxTau))
thrd1.start()
...
MaxV = math.sqrt(MaxV**2 + V0**2)
MaxEps = math.sqrt(MaxEps**2 + Eps0**2)
if (not isVelocity):
	MaxV = MaxEps
plt = open('plotter','r')
